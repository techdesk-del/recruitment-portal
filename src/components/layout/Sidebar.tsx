...
export const Sidebar: React.FC = () => {
  ...
  const queuedCallsCount = candidates.filter((c) => c.isCallingQueued || c.callingDetails?.callStatus === 'in_progress').length;
  const callingCount = queuedCallsCount > 0 ? queuedCallsCount : (callingMetrics?.pendingCallsCount || 4);
      ...
      <div className="p-3.5 space-y-5 overflow-y-auto">
        ...
        <div className="space-y-1">
          ...
          <button
            ...
            className={`w-full flex items-center justify-between px-3 py-2 rounded-xl text-[13px] transition-all tracking-tight ${
              activeView === 'calling' || activeView === 'calling-desk'
                ? 'bg-[#2563eb] text-white font-medium shadow-xs'
                : 'text-slate-500 hover:text-slate-900 hover:bg-slate-50 font-normal'
            }`}
          >
            <div className="flex items-center gap-2.5">
              <PhoneCall size={16} className={activeView === 'calling' || activeView === 'calling-desk' ? 'text-white' : 'text-emerald-600'} />
              <span>Calling</span>
            </div>
            <span className={`text-[10px] font-bold px-1.5 py-0.2 rounded-full ${
              activeView === 'calling' || activeView === 'calling-desk' ? 'bg-blue-800/80 text-white' : 'bg-emerald-50 text-emerald-700'
            }`}>
              {callingCount}
            </span>
          </button>
        ...
        {/* Employee Referral Section directly below Sourcing Portals */}
        <div className="pt-2.5 border-t border-slate-100 space-y-0.5">
          ...
          {/* All Referrals item */}
          <button
            onClick={() => handleReferralPortalClick()}
            className={`w-full flex items-center justify-between px-3 py-1.5 rounded-lg text-[13px] transition-all tracking-tight ${
              activeView === 'referral'
          ...
          >
            <div className="flex items-center gap-2.5">
              <PortalLogo source="referral" size={17} />
              <span>All Referrals</span>
            </div>
            <span className={`text-[11px] font-normal ${
              activeView === 'referral' ? 'text-purple-600 font-semibold' : 'text-slate-400'
            }`}>
              {getPortalCount('referral')}
            </span>
          </button>
        </div>
